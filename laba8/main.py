...
class MainWindow(QWidget):
    ...
    def _connect_to_db(self):
        ...
        self.conn = psycopg2.connect(database="postgres", user="postgres", password="paketik123", host="localhost",
                                     port="5432")
    ...
    def _create_shedule_tab(self):
        ...
        self.tabs_days1.addTab(self.monday_tab, "Понедельник")
        self.Tuesday_tab = QWidget(self)
        self.tabs_days1.addTab(self.Tuesday_tab, "Вторник")
        self.Wednesday_tab = QWidget(self)
        self.tabs_days1.addTab(self.Wednesday_tab, "Среда")
        self.Thursday_tab = QWidget(self)
        self.tabs_days1.addTab(self.Thursday_tab, "Четверг")
        self.Friday_tab = QWidget(self)
        self.tabs_days1.addTab(self.Friday_tab, "Пятница")
    ...
    def _create_teacher_table(self):
        ...
        self.teacher_table.setHorizontalHeaderLabels(["ID", "Имя", "Предмет"])
    ...
    def _create_monday_table(self):
        ...
        self.monday_table.setHorizontalHeaderLabels(
            ["Неделя", "Предмет", "Аудитория", "Время", "Join", 'Delete'])
    ...
    def _update_monday_table(self):
        ...
        self.cursor.execute(
            "SELECT week, subject, room_numb, start_time FROM timetable WHERE day='Понедельник' ORDER BY 1 DESC;")
        records = list(self.cursor.fetchall())
    ...
    def _delete_day_from_table(self, day, data):
        ...
        if day == 'subject':
            try:
                ...
                self.conn.commit()
                self.cursor.execute(f"DELETE FROM subjects where name='{data[0]}';")
                self.conn.commit()
            ...
            try:
                self.cursor.execute(
                    f"DELETE FROM teacher where id={int(data[0])} AND full_name='{data[1]}' AND subject='{data[2]}';")
                self.conn.commit()
                self._update_shedule()
            except:
            ...
            try:
                self.cursor.execute(
                    f"delete from timetable where day ='{day}' AND week='{data[0]}' AND subject='{data[1]}' AND room_numb='{data[2]}' AND start_time='{data[3]}';")
                self.conn.commit()
                self._update_shedule()
            except:
    ...
    def _add_row_tabale(self, rowNum, day):
        ...
        match day:
            ...
            case 'subject':
                ...
                try:
                    self.cursor.execute(f"INSERT INTO subjects (name) VALUES ('{row[0]}');")
                    self.conn.commit()
                    self._update_shedule()
                except:
            ...
            case 'teacher':
                ...
                try:
                    ...
                    self.cursor.execute(f"INSERT INTO teacher ("
                                        f"full_name, subject) VALUES ('{row[1]}', '{row[2]}');")
                    self.conn.commit()
                    self._update_shedule()
                except:
        ...
        if '' not in row[:-2:] and day != 'subject' and day != 'teacher':
            ...
            try:
                self.cursor.execute(
                    f"INSERT INTO timetable (week, day, subject, room_numb, start_time) VALUES ("
                    f"'{row[0]}', '{day}', '{row[1]}', '{row[2]}', '{row[3]}');")
                self.conn.commit()
                self._update_shedule()
            except:
    ...
    def _update_Tuesday_table(self):
        ...
        self.cursor.execute(
            "SELECT week, subject, room_numb, start_time FROM timetable WHERE day='Вторник' ORDER BY 1 DESC;")
        records = list(self.cursor.fetchall())
    ...
    def _update_Wednesday_table(self):
        ...
        self.cursor.execute(
            "SELECT week, subject, room_numb, start_time FROM timetable WHERE day='Среда' ORDER BY 1 DESC;")
        records = list(self.cursor.fetchall())
    ...
    def _update_Thursday_table(self):
        ...
        self.cursor.execute(
            "SELECT week, subject, room_numb, start_time FROM timetable WHERE day='Четверг' ORDER BY 1 DESC;")
        records = list(self.cursor.fetchall())
    ...
    def _update_Friday_table(self):
        ...
        self.cursor.execute(
            "SELECT week, subject, room_numb, start_time FROM timetable WHERE day='Пятница' ORDER BY 1 DESC;")
        records = list(self.cursor.fetchall())
    ...
    def _change_day_from_table(self, rowNum, day, data):
        ...
        match day:
            ...
            case 'subject':
                ...
                if row[0] != data[0]:
                    try:
                        ...
                        self.cursor.execute(f"DELETE FROM timetable where subject='{data[0]}';")
                        self.conn.commit()
                        self.cursor.execute(f"UPDATE subjects SET name='{row[0]}' where name='{data[0]}';")
                        self.conn.commit()
                        self._update_shedule()
                    except:
            ...
            case 'teacher':
                ...
                if row[:-2:] != data[0]:
                    try:
                        self.cursor.execute(
                            f"UPDATE teacher SET id={int(row[0])}, full_name='{row[1]}', subject='{row[2]}' where id ={int(data[0])} AND full_name='{data[1]}' AND subject='{data[2]}';")
                        self.conn.commit()
                        self._update_shedule()
                    except:
        ...
        if data != row[:-2:] and day != 'subject' and day != 'teacher':
            ...
            try:
                self.cursor.execute(
                    f"UPDATE timetable SET week='{row[0]}', subject='{row[1]}', room_numb ='{row[2]}', start_time='{row[3]}' WHERE day='{day}' AND week='{data[0]}' AND subject = '{data[1]}'AND room_numb = '{data[2]}' AND start_time = '{data[3]}';")
                self.conn.commit()
                self._update_shedule()
            except:
